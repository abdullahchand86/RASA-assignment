...
class TestActionGetLocation:
    ...
    def test_successful_geocode_sets_slots(self, mock_get):
        ...
        action = ActionGetLocation()
        dispatcher = FakeDispatcher()
        tracker = FakeTracker(slots={"destination": "Paris"})
...
class TestActionCalculateCarbon:
    ...
    def test_api_failure_sets_unknown_tier_not_crash(self, mock_post):
        action = ActionCalculateCarbon()
        dispatcher = FakeDispatcher()
        ...
        assert slot_values["carbon_tier"] == "unknown"
        assert slot_values["carbon_estimate"] is None
